...
When the close is complete C<socket.on_close()> is made.
...
=item void oi_server_init (oi_server *, int max_connections);

Initializes a server object. C<max_connections> is the argument given
